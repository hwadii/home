skip_global_compinit=1
export LOCALE_ARCHIVE=/usr/lib/locale/locale-archive
export LANGUAGE=en_US.UTF-8
export LANG=en_US
export LC_ALL=en_US.UTF-8
export TERMINAL="foot"
export SNAP="/snap"
export CHROME_BIN="google-chrome-stable"
export XDG_CACHE_HOME="$HOME/.cache"
export XDG_LOCAL_BIN="$HOME/.local/bin"
export XDG_DATA_DIR="$HOME/.local/share"
export XDG_DATA_HOME="$HOME/.local/share"
export XDG_MISC_DIR="$HOME/.local/misc"
export TMPDIR="/tmp"
export BROWSER="firefox"
export LESS="--RAW-CONTROL-CHARS --mouse -Ri"
export PAGER="less"
export XDG_CONFIG_HOME="$HOME/.config"
export ZDOTDIR="$XDG_CONFIG_HOME/zsh"
export RIPGREP_CONFIG_PATH="$XDG_CONFIG_HOME/ripgrep/ripgreprc"
export CARGO_HOME="$HOME/.local/cargo"
export GOPATH="$HOME/.local/go"
export RUSTUP_HOME="$HOME/.local/rustup"
export RUBY_YJIT_ENABLE=1
export DOTNET_ROOT=$(rtx where dotnet-core)
export PATH="$XDG_LOCAL_BIN:$PATH:$XDG_LOCAL_BIN/git-extras:$HOME/.local/texlive/2020/bin/x86_64-linux:$CARGO_HOME/bin:$GOPATH/bin:$HOME/.local/rustup/toolchains/stable-x86_64-unknown-linux-gnu/bin:$HOME/.dotnet/tools"
export EDITOR=nvim
export VISUAL=nvim
export FZF_DEFAULT_COMMAND='fd --type f --follow --hidden -E".git"'
export FZF_DEFAULT_OPTS="--height 40% --bind 'tab:down' --bind 'btab:up' --bind 'ctrl-s:toggle' --bind 'alt-a:select-all' --color=16,hl:3,hl+:6,fg+:-1,pointer:6,bg+:-1"
export FZF_CTRL_T_OPTS="--select-1 --exit-0"
export FZF_ALT_C_COMMAND="fd --type d --hidden --follow --exclude '.git' --exclude 'node_modules'"
export FZF_ALT_C_OPTS="--preview 'tree -C {} | head -200'"
export SSH_AUTH_SOCK="$XDG_RUNTIME_DIR/keyring/ssh"
export PASSWORD_STORE_DIR="$XDG_DATA_DIR/pass"
export NNN_BMS="d:$HOME/dl;b:$HOME/things/books;c:$HOME/code;s:/media/wadii/garbage"
export MANWIDTH=80
export JQ_COLORS="2;37:0;37:0;37:0;37:0;32:1;37:1;37"
if [ -e /home/wadii/.nix-profile/etc/profile.d/nix.sh ]; then . /home/wadii/.nix-profile/etc/profile.d/nix.sh; fi # added by Nix installer
